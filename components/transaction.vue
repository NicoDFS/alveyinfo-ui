<template>
  <div class="columns is-multiline transaction-item">
        ...
        <div v-for="input in inputs" class="is-clearfix">
          ...
          <div class="script">
            ...
            <div v-if="input.scriptSig.asm">
              ...
              <code class="value"><!--
                -->{{ input.scriptSig.asm | alvey-script }}<!--
              --></code>
            </div>
          ...
          <div class="script">
            ...
            <div v-if="output.scriptPubKey.asm">
              ...
              <code class="value" :class="{'script-code': contractInfo[index]}"
                ...
                -->{{ output.scriptPubKey.asm | alvey-script }}<!--
              --></code>
            </div>
    ...
    <template v-for="({address, name, symbol, decimals, from, to, value}, index) in arc20TokenTransfers">
      <div class="column is-full flex-full"></div>
      <AttributeInjector
        class="column collapse token-transfer-list"
        :class="{
          'first-item': index === 0,
          'last-item': index === arc20TokenTransfers.length - 1
        }">
        <div class="is-clearfix">
    ...
    <div class="column is-full has-text-right collapse-bottom" v-if="fees !== '0'">
      <template v-if="fees > 0">
        {{ $t('transaction.fee') }} <span class="amount fee">{{ fees | alvey }} ALVEY</span>
      </template>
      <template v-else-if="fees < 0">
        {{ $t('transaction.reward') }} <span class="amount fee">{{ -fees | alvey }} ALVEY</span>
      </template>
    </div>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      arc20TokenTransfers() {
        return this.transaction.arc20TokenTransfers
      },
